...
export async function createComment(ctx) {
  const { content, ipfsHash, contentHash, userId, parentId, tagNames } = ctx.request.body
...
export async function listComments(ctx) {
  ...
  const comments = await prisma.comment.findMany({
    ...
    orderBy: sort === 'hot' ? { likesCount: 'desc' } : { createdAt: 'desc' },
    include: {
...
export async function getThread(ctx) {
